...
void Game::drawWindow(sf::RenderWindow &window)
{
    ...
    while (window.isOpen())
    {
        ...
        //Neural Play Mode:
        if(m_f_mo.get_mode()==0)
            dir = get_direction(s[0],f);

        //Manual Play Mode:
		else if(m_f_mo.get_mode()==1)
        {
...
float Game:: distance(int p,int q,fruit b)
{
    float d;
...
//Takes an coordinate to check whether it is safe(1) or danger(0):
...
int Game:: get_direction(snake sh,fruit fr)
{
    ...
    if(check(sh.x-1,sh.y) == 1)     l=distance(sh.x-1,sh.y,f);    //Check Left coordinate
    else    l=1000;

    if(check(sh.x,sh.y-1) == 1)     u=distance(sh.x,sh.y-1,f);    //Check Up coordinate
    else    u=1001;

    if(check(sh.x+1,sh.y) == 1)     r=distance(sh.x+1,sh.y,f);    //Check Right coordinate
    else    r=1002;

    if(check(sh.x,sh.y+1) == 1)     d=distance(sh.x,sh.y+1,f);     //Check Down coordinate
    else    d=1003;
